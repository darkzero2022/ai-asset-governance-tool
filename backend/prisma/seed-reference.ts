...
export async function seedReferenceData() {
  ...
  const frameworkCategories = [
    ...
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP04", name: "Software Supply Chain Attacks & Dependency Tampering", description: "Compromised MCP server dependencies introduce backdoors or alter agent behaviour at runtime." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP05", name: "Command Injection & Execution", description: "Untrusted input reaches system commands, scripts, or API calls an agent executes without validation." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP06", name: "Prompt Injection via Contextual Payloads", description: "Natural-language payloads delivered through tool output or resources subvert the model's instructions." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP07", name: "Insufficient Authentication & Authorization", description: "Weak identity verification and access control between agents, clients, and MCP servers create attack paths." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP08", name: "Lack of Audit and Telemetry", description: "Limited logging of tool invocations and context changes impedes detection and incident response." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP09", name: "Shadow MCP Servers", description: "Unapproved MCP deployments run outside governance with default credentials and permissive configs." },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP10", name: "Context Injection & Over-Sharing", description: "Shared or persistent context windows leak sensitive data across tasks, users, or agents." },
  ] as const;
  ...
  const frameworkThreatMappings = [
    ...
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM01", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["LLM Prompt Injection"], atlasMitigations: [M("0020"), M("0015")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM02", strideAiCategory: "MODEL_INVERSION", atlasTechniques: ["Exfiltration via Inference API"], atlasMitigations: [M("0012"), M("0000"), M("0019")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM03", strideAiCategory: "MODEL_IMPERSONATION", atlasTechniques: ["ML Supply Chain Compromise"], atlasMitigations: [M("0013"), M("0014"), M("0016")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM04", strideAiCategory: "DATA_MODEL_POISONING", atlasTechniques: ["Data Poisoning"], atlasMitigations: [M("0007"), M("0008")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM05", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["Downstream Execution of Unvalidated Output"], atlasMitigations: [M("0015"), M("0020")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM06", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["Exfiltration via AI Agent Tool Invocation"], atlasMitigations: [M("0019"), M("0020")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM07", strideAiCategory: "PROVENANCE_LOSS", atlasTechniques: ["Discovery: LLM System Prompt"], atlasMitigations: [M("0000"), M("0019")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM08", strideAiCategory: "DATA_MODEL_POISONING", atlasTechniques: ["RAG Poisoning / False RAG Entry Injection"], atlasMitigations: [M("0007"), M("0019")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM09", strideAiCategory: "MODEL_INVERSION", atlasTechniques: [], atlasMitigations: [M("0021"), M("0022")] },
    { framework: "OWASP_LLM_TOP10", categoryId: "LLM10", strideAiCategory: "RESOURCE_EXHAUSTION", atlasTechniques: ["Sponge Example / Context Flooding"], atlasMitigations: [M("0004")] },
    // OWASP MCP Top 10 (draft v0.1) — STRIDE-AI is closest-fit for MCP01/02/05/07/08/09.
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP01", strideAiCategory: "PROVENANCE_LOSS", atlasTechniques: ["Unsecured Credentials"], atlasMitigations: [M("0012"), M("0005")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP02", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["Exfiltration via AI Agent Tool Invocation"], atlasMitigations: [M("0019"), M("0015")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP03", strideAiCategory: "DATA_MODEL_POISONING", atlasTechniques: ["LLM Plugin Compromise"], atlasMitigations: [M("0014"), M("0013")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP04", strideAiCategory: "MODEL_IMPERSONATION", atlasTechniques: ["ML Supply Chain Compromise"], atlasMitigations: [M("0013"), M("0014"), M("0016")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP05", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["Downstream Execution of Unvalidated Output"], atlasMitigations: [M("0015"), M("0020")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP06", strideAiCategory: "ALIGNMENT_BYPASS", atlasTechniques: ["LLM Prompt Injection"], atlasMitigations: [M("0020"), M("0015")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP07", strideAiCategory: "MODEL_IMPERSONATION", atlasTechniques: ["Valid Accounts"], atlasMitigations: [M("0019"), M("0005")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP08", strideAiCategory: "PROVENANCE_LOSS", atlasTechniques: [], atlasMitigations: [M("0019")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP09", strideAiCategory: "MODEL_IMPERSONATION", atlasTechniques: ["Masquerading"], atlasMitigations: [M("0019"), M("0016")] },
    { framework: "OWASP_MCP_TOP10", categoryId: "MCP10", strideAiCategory: "MODEL_INVERSION", atlasTechniques: ["RAG Poisoning / False RAG Entry Injection"], atlasMitigations: [M("0020"), M("0012")] },
  ] as const;
  ...
  for (const mapping of frameworkThreatMappings) {
    ...
    await prisma.frameworkThreatMapping.upsert({
      where: { framework_categoryId: { framework: mapping.framework, categoryId: mapping.categoryId } },
      update: data,
  ...
  const crosswalkSeeds = [
    { from: ["OWASP_MCP_TOP10", "MCP06"], to: ["OWASP_LLM_TOP10", "LLM01"], relationship: "EQUIVALENT", rationale: "Both are prompt injection; MCP06 is the same class delivered through tool output / MCP resources." },
    { from: ["OWASP_MCP_TOP10", "MCP03"], to: ["OWASP_LLM_TOP10", "LLM03"], relationship: "RELATED", rationale: "A poisoned tool is a compromised third-party component in the model's supply chain." },
    { from: ["OWASP_MCP_TOP10", "MCP04"], to: ["OWASP_LLM_TOP10", "LLM03"], relationship: "RELATED", rationale: "MCP server dependency tampering is a supply-chain compromise." },
    { from: ["OWASP_MCP_TOP10", "MCP01"], to: ["OWASP_LLM_TOP10", "LLM02"], relationship: "RELATED", rationale: "Leaked tokens/secrets are sensitive information disclosed through context, memory, or logs." },
    { from: ["OWASP_MCP_TOP10", "MCP10"], to: ["OWASP_LLM_TOP10", "LLM02"], relationship: "RELATED", rationale: "Over-shared context windows disclose sensitive information across tasks/users." },
    { from: ["OWASP_MCP_TOP10", "MCP10"], to: ["OWASP_LLM_TOP10", "LLM08"], relationship: "RELATED", rationale: "Persistent/shared retrieval context is a vector & embedding weakness." },
    { from: ["OWASP_MCP_TOP10", "MCP02"], to: ["OWASP_LLM_TOP10", "LLM06"], relationship: "RELATED", rationale: "Scope creep on tool permissions is excessive agency." },
    { from: ["OWASP_MCP_TOP10", "MCP05"], to: ["OWASP_LLM_TOP10", "LLM05"], relationship: "RELATED", rationale: "Command injection stems from executing unvalidated model/tool output." },
    { from: ["OWASP_MCP_TOP10", "MCP08"], to: ["NIST_AI_RMF", "MANAGE"], relationship: "RELATED", rationale: "Audit & telemetry gaps are a failure to monitor and respond (NIST MANAGE)." },
    { from: ["OWASP_MCP_TOP10", "MCP08"], to: ["NIST_AI_RMF", "GOVERN"], relationship: "RELATED", rationale: "Logging/accountability of AI tooling is a governance control." },
    { from: ["OWASP_MCP_TOP10", "MCP07"], to: ["NIST_AI_RMF", "MAP"], relationship: "RELATED", rationale: "Identifying trust boundaries and access requirements between agents/servers is a MAP activity." },
    { from: ["OWASP_MCP_TOP10", "MCP09"], to: ["NIST_AI_RMF", "MAP"], relationship: "RELATED", rationale: "Discovering unsanctioned MCP deployments is context/inventory mapping." },
  ] as const;

  const reverseRelationship = (r: string) => (r === "BROADER" ? "NARROWER" : r === "NARROWER" ? "BROADER" : r);
  const crosswalkRows = crosswalkSeeds.flatMap((seed) => [
    { fromFramework: seed.from[0], fromCategoryId: seed.from[1], toFramework: seed.to[0], toCategoryId: seed.to[1], relationship: seed.relationship, rationale: seed.rationale },
    { fromFramework: seed.to[0], fromCategoryId: seed.to[1], toFramework: seed.from[0], toCategoryId: seed.from[1], relationship: reverseRelationship(seed.relationship), rationale: seed.rationale },
  ]);
  ...
  const frameworkMeta = [
    { framework: "NIST_AI_RMF", title: "NIST AI Risk Management Framework", revision: "NIST AI 100-1 (Jan 2023)", status: "RELEASED", sourceUrl: "https://www.nist.gov/itl/ai-risk-management-framework", licenseNote: "U.S. Government work — not subject to copyright in the United States." },
    { framework: "EU_AI_ACT", title: "EU Artificial Intelligence Act", revision: "Regulation (EU) 2024/1689", status: "RELEASED", sourceUrl: "https://eur-lex.europa.eu/eli/reg/2024/1689/oj", licenseNote: "© European Union, https://eur-lex.europa.eu — reuse authorised with acknowledgement." },
    { framework: "OWASP_LLM_TOP10", title: "OWASP Top 10 for LLM Applications", revision: "2025", status: "RELEASED", sourceUrl: "https://genai.owasp.org/llm-top-10/", licenseNote: "OWASP — CC BY-SA 4.0." },
    { framework: "OWASP_MCP_TOP10", title: "OWASP Top 10 for MCP (Model Context Protocol)", revision: "Draft v0.1 (2025)", status: "DRAFT", sourceUrl: "https://github.com/OWASP/www-project-mcp-top-10", licenseNote: "OWASP — CC BY-NC-SA 4.0 (non-commercial). Draft — categories and wording may change." },
  ] as const;

  for (const meta of frameworkMeta) {
    await prisma.frameworkMeta.upsert({
      where: { framework: meta.framework },
      update: { title: meta.title, revision: meta.revision, status: meta.status, sourceUrl: meta.sourceUrl, licenseNote: meta.licenseNote },
      create: meta,
    });
  }
}
