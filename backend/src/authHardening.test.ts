...
describe("change password", () => {
  it("enforces the policy, checks the current password, and invalidates every existing session", async () => {
    ...
    await request(app)
      .post("/api/v1/auth/change-password")
      .set("Authorization", `Bearer ${oldAccess}`)
      .send({ currentPassword: password, newPassword: "welcome12345" })
      .expect(422);
    // wrong current
    await request(app)
      .post("/api/v1/auth/change-password")
      .set("Authorization", `Bearer ${oldAccess}`)
      .send({ currentPassword: "not-the-current-one", newPassword: "brand-new-solid-passphrase" })
      .expect(401);
    ...
    const changed = await request(app)
      .post("/api/v1/auth/change-password")
      .set("Authorization", `Bearer ${oldAccess}`)
      .send({ currentPassword: password, newPassword: "brand-new-solid-passphrase" })
      .expect(200);
    ...
    await request(app)
      .post("/api/v1/auth/login")
      .send({ email, password: "brand-new-solid-passphrase" })
      .expect(200);
  });
});
