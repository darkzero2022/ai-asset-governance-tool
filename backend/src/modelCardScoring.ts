...
export function modelCardCompleteness(card: ModelCardLike | null | undefined) {
  ...
  if (!hasValue(card?.architectureFamily) && !hasValue(card?.modelArchitecture)) missingFields.push("architecture");
  ...
  return { percent: Math.round((completeCount / REQUIRED_MODEL_CARD_FIELDS.length) * 100), missingFields };
}
