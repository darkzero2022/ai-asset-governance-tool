...
type AssetLike = { risks?: Array<{ id: string; sourceFramework: string; sourceCategoryId: string }> };
...
export async function buildBomFrameworkContext(assets: AssetLike[]): Promise<BomFrameworkContext> {
  ...
  const meta = await prisma.frameworkMeta.findMany({ where: { framework: { in: frameworks as never } } });
  ...
  for (const risk of risks) {
    ...
    if (!cache.has(key)) cache.set(key, await relatedClassificationsFor(risk.sourceFramework, risk.sourceCategoryId));
    const related = cache.get(key)!;
    if (related.length) {
      relatedByRisk[risk.id] = related.map((row) => ({ framework: row.framework, categoryId: row.categoryId, relationship: row.relationship }));
    }
  }
