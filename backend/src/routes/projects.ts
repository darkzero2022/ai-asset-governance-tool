...
router.get("/projects", requireAuth, async (req, res, next) => {
  try {
    ...
    const projects = await prisma.project.findMany({
      where,
      include: {
        _count: { select: { assetLinks: true, riskLinks: true } },
...
router.post(
  "/projects/:projectId/ai-systems/:assetId",
  requireAuth,
  requireRole("ADMIN", "RISK_OWNER"),
  async (req, res, next) => {
    try {
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const projectId = String(req.params.projectId);
      const assetId = String(req.params.assetId);
      await prisma.projectAsset.findUniqueOrThrow({
        where: { projectId_assetId: { projectId, assetId } },
      });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const link = await prisma.projectRisk.upsert({
        where: { projectId_riskId: { projectId, riskId } },
        update: {},
        create: { projectId, riskId },
      });
      res.status(201).json({ link });
    } catch (error) {
      next(error);
...
router.delete(
  ...
  async (req, res, next) => {
    try {
      const projectId = String(req.params.projectId);
      const riskId = String(req.params.riskId);
      await prisma.projectRisk.findUniqueOrThrow({
        where: { projectId_riskId: { projectId, riskId } },
      });
...
router.get("/projects/:id/risks", requireAuth, async (req, res, next) => {
  try {
    ...
    for (const link of project.riskLinks)
      risks.set(link.riskId, { ...riskResponse(link.risk), origin: "project" });
    for (const assetLink of project.assetLinks) {
      for (const riskLink of assetLink.asset.riskLinks) {
        if (!risks.has(riskLink.riskId))
          risks.set(riskLink.riskId, { ...riskResponse(riskLink.risk), origin: "asset" });
      }
    }
