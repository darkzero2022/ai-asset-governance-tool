  ...
  if (!props.project) {
    return (
      <section className="mx-auto max-w-7xl px-6 py-6">
        ...
        <Card><CardBody>Loading project…</CardBody></Card>
      </section>
    );
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="min-w-0 space-y-6">
            ...
            <CardBody>
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-subtle">{props.label(props.project.status)} · Owner: {props.project.businessOwner || "Unassigned"}</p>
                </div>
                <Button variant="secondary" size="sm" onClick={props.onExportCycloneDx}>
                  <Download className="h-3.5 w-3.5" /> Export CycloneDX
                </Button>
              </div>
              <p className="mt-4 text-sm text-text">{props.project.description || "No description"}</p>
            </CardBody>
          </Card>
          ...
          <Panel title="Linked AI Systems">
            ...
            <div className="mt-4 space-y-3">
              ...
              {!(props.project.assetLinks ?? []).length && <EmptyState title="No linked AI systems." />}
            </div>
          </Panel>
        </div>
        ...
        <aside className="min-w-0 space-y-6">
          <Panel title="Direct Risks">
            {props.canManage && (
              <div className="flex gap-2">
                <div className="min-w-0 flex-1"><Combobox value={props.selectedRiskId} onChange={props.onSelectedRiskChange} options={availableRisks.map((risk) => ({ value: risk.id, label: risk.description }))} placeholder="Select risk" /></div>
            ...
            <div className="space-y-3">
              {props.mergedRisks.map((risk) => (
                <div key={risk.id} className="rounded-md border border-border p-3 text-sm">
                  ...
                  <p className="text-subtle">{props.label(risk.status)} · Score {risk.inherentRiskScore} · Origin: {risk.origin ?? "project"}</p>
                </div>
              ))}
  ...
  return (
    <div className="rounded-md border border-border p-3 text-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0"><p className="break-words font-medium text-text">{props.title}</p><p className="text-subtle">{props.subtitle}</p></div>
        {props.onRemove && <button className="shrink-0 text-xs font-semibold text-danger hover:underline" onClick={props.onRemove}>Unlink</button>}
      </div>
    </div>
  );
}
