...
export default function AiSystemList(props: Props) {
  ...
  const updateForm = (key: keyof AssetForm, value: string) =>
    props.onFormChange({ ...props.assetForm, [key]: value });
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 py-6">
      <PageHeader
        ...
        action={
          props.canManage && (
            <Button variant="primary" onClick={props.onNewAiSystem}>
              New AI system
            </Button>
          )
        }
        filters={
          <div className="grid gap-3 md:grid-cols-4">
            <Select
              label="Status"
              value={props.filters.assetStatus}
              onChange={(event) => updateFilter("assetStatus", event.target.value)}
            >
              {STATUS_OPTIONS.map((option) => (
                <option key={option} value={option}>
                  {option ? props.label(option) : "All"}
                </option>
              ))}
            </Select>
            <Select
            ...
            <Select
              label="Hosting"
              value={props.filters.hostingModel}
              onChange={(event) => updateFilter("hostingModel", event.target.value)}
            >
              <option value="">All</option>
            ...
            <Select
              label="Network"
              value={props.filters.networkDependency}
              onChange={(event) => updateFilter("networkDependency", event.target.value)}
            >
              <option value="">All</option>
            ...
            <div className="max-h-[65vh] space-y-4 overflow-y-auto pr-1">
              ...
              <div className="rounded-md border border-border p-3">
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    variant="secondary"
                    className="self-end"
                    disabled={!props.assetForm.sourceUrl}
                    onClick={() => props.onFetchImport(props.assetForm.sourceUrl)}
                  >
                    Fetch
                  </Button>
                </div>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Select
                  ...
                  onChange={(event) => updateForm("hostingModel", event.target.value)}
                >
                  {HOSTING_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {props.label(option)}
                ...
                <Select
                  ...
                  onChange={(event) => updateForm("networkDependency", event.target.value)}
                >
                  {NETWORK_OPTIONS.map((option) => (
                    <option key={option} value={option}>
                      {props.label(option)}
                ...
                <Input
                  ...
                  onChange={(event) => updateForm("dataClassificationTouched", event.target.value)}
                />
              </div>
              <TextArea
                label="Training Data Provenance"
                value={props.assetForm.trainingDataProvenance ?? ""}
                onChange={(event) => updateForm("trainingDataProvenance", event.target.value)}
              />
              <TextArea
                label="Downstream Consumers"
                value={props.assetForm.downstreamConsumers ?? ""}
                onChange={(event) => updateForm("downstreamConsumers", event.target.value)}
              />
            </div>
            <DialogFooter>
              ...
              <Button type="submit" variant="primary" size="sm">
                {props.editingAssetId ? "Save AI system" : "Create AI system"}
              </Button>
            </DialogFooter>
          </form>
        </Dialog>
