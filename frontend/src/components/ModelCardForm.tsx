...
export function modelCardToForm(modelCard: ModelCard | null): ModelCardFormState {
  return {
    ...
    performanceMetrics: modelCard?.performanceMetrics
      ? JSON.stringify(modelCard.performanceMetrics, null, 2)
      : "",
    metrics: (modelCard?.metrics ?? []).map((metric) => ({
      id: metric.id,
...
export function ModelCardForm({
  ...
  const update = (key: keyof ModelCardFormState, value: string) =>
    onFormChange({ ...form, [key]: value });
  const updateMetric = (index: number, key: keyof ModelCardMetricFormState, value: string) =>
    onFormChange({
      ...form,
      metrics: form.metrics.map((metric, metricIndex) =>
  ...
  const addMetric = () =>
    onFormChange({
      ...form,
      metrics: [...form.metrics, { metricName: "", metricValue: "", slice: "" }],
    });
  const removeMetric = (index: number) =>
    onFormChange({
      ...form,
      metrics: form.metrics.filter((_metric, metricIndex) => metricIndex !== index),
    });
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-subtle">
            Required before approving or deploying MODEL and SERVICE AI systems.
          </p>
        </div>
        {displayCompleteness && (
          <Badge variant="primary">{displayCompleteness.percent}% complete</Badge>
        )}
      </div>
      {displayCompleteness?.missingFields?.length ? (
        <p className="mt-3 text-sm text-warning">
          Missing: {displayCompleteness.missingFields.join(", ")}
        </p>
      ) : null}
      <div className="mt-4 rounded-md border border-border p-4">
        <div className="flex gap-2">
          ...
          <Button
            ...
            className="self-end"
            disabled={!sourceUrl}
            onClick={() => onFetchImport(sourceUrl)}
          >
            Fetch
          </Button>
        </div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <Input
          ...
          value={form.architectureFamily}
          onChange={(event) => update("architectureFamily", event.target.value)}
        />
        <Input
          label="Model Architecture"
          value={form.modelArchitecture}
          onChange={(event) => update("modelArchitecture", event.target.value)}
        />
        <TextArea
          label="Datasets"
          value={form.datasetsDescription}
          onChange={(event) => update("datasetsDescription", event.target.value)}
        />
        <TextArea
          label="Inputs"
          value={form.inputsDescription}
          onChange={(event) => update("inputsDescription", event.target.value)}
        />
        <TextArea
          label="Outputs"
          value={form.outputsDescription}
          onChange={(event) => update("outputsDescription", event.target.value)}
        />
        <TextArea
          label="Intended Users"
          value={form.intendedUsers}
          onChange={(event) => update("intendedUsers", event.target.value)}
        />
        <TextArea
          label="Use Cases"
          value={form.useCases}
        ...
        <TextArea
          ...
          onChange={(event) => update("technicalLimitations", event.target.value)}
        />
        <TextArea
          label="Performance Tradeoffs"
          value={form.performanceTradeoffs}
          onChange={(event) => update("performanceTradeoffs", event.target.value)}
        />
        <TextArea
          label="Ethical Considerations"
          value={form.ethicalConsiderations}
          onChange={(event) => update("ethicalConsiderations", event.target.value)}
        />
        <TextArea
          label="Fairness Assessments"
          value={form.fairnessAssessments}
          onChange={(event) => update("fairnessAssessments", event.target.value)}
        />
        <TextArea
          label="Environmental Considerations"
          value={form.environmentalConsiderations}
          onChange={(event) => update("environmentalConsiderations", event.target.value)}
        />
        <TextArea
          label="Performance Metrics JSON"
          value={form.performanceMetrics}
          onChange={(event) => update("performanceMetrics", event.target.value)}
      ...
      <div className="mt-5 rounded-md border border-border p-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-subtle">
              Use normalized rows for cross-model dashboard comparisons.
            </p>
          </div>
        ...
        <div className="mt-3 space-y-3">
          {form.metrics.map((metric, index) => (
            <div key={metric.id ?? index} className="grid gap-2 md:grid-cols-[1fr_0.6fr_1fr_auto]">
              <Input
                ...
                onChange={(event) => updateMetric(index, "metricName", event.target.value)}
              />
              <Input
                label="Value"
                value={metric.metricValue}
                onChange={(event) => updateMetric(index, "metricValue", event.target.value)}
              />
              <Input
                label="Slice"
                value={metric.slice}
                onChange={(event) => updateMetric(index, "slice", event.target.value)}
              />
              <Button
                type="button"
                variant="danger"
          ...
          {!form.metrics.length && (
            <p className="text-sm text-subtle">No structured metrics yet.</p>
          )}
        </div>
      </div>
  ...
  return (
    <div className="mt-3 rounded-md bg-surface-alt p-3 text-xs text-subtle ring-1 ring-border">
      ...
      <p className="mt-1">
        ...
        {props.suggestion.suggestedTitle || "Not found"}
      </p>
      <p className="mt-1">
        <span className="font-medium text-text">Description:</span>{" "}
        {props.suggestion.suggestedDescription || "Not found"}
      </p>
      <p className="mt-1">
        <span className="font-medium text-text">Excerpt:</span>{" "}
        {props.suggestion.excerpt || "Not found"}
      </p>
    </div>
  );
}
