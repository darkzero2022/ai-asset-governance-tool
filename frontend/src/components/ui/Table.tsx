...
export function Thead({ children }: { children: ReactNode }) {
  return <thead className="sticky top-0 z-10 bg-surface-alt text-xs font-semibold uppercase tracking-wide text-subtle">{children}</thead>;
}

export function Tbody({ children }: { children: ReactNode }) {
  return <tbody className="divide-y divide-border [&>tr:nth-child(even)]:bg-surface-alt/40">{children}</tbody>;
}
